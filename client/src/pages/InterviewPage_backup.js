...
const InterviewPage = () => {
  ...
  const completeInterview = async (finalAnswers) => {
    setIsCompleted(true);
    setLoading(true); // Show loading while calculating AI scores
    
    try {
      // Calculate score using AI analysis
      const score = await calculateScore(finalAnswers);
      
      const interviewResult = {
        testType,
        difficulty,
        mode,
        questions: questions.map(q => q._id || q.question), // Handle both DB and AI questions
      ...
      setResults({
        ...
        answers: finalAnswers,
        analysisResults: window.lastAnalysisResults || []
      });
    } catch (error) {
      console.error('Error completing interview:', error);
      // Fallback to basic scoring if AI fails
      const basicScore = calculateBasicScore(finalAnswers.map(a => a.answer).join(' '));
      setResults({
        score: basicScore,
        totalQuestions: questions.length,
  ...
  const calculateScore = async (answers) => {
    ...
    try {
      // Use AI analysis for each answer
      for (let i = 0; i < answers.length; i++) {
        ...
        if (question && answer.answer.trim()) {
          try {
            ...
            const response = await fetch('http://localhost:5000/api/interview/analyze-answer', {
              method: 'POST',
              headers: {
            ...
            if (response.ok) {
              ...
              if (result.success && result.analysis) {
                ...
                const basicScore = calculateBasicScore(answer.answer);
                totalScore += basicScore;
                analysisResults.push({ score: basicScore, feedback: 'Basic analysis applied' });
              }
            } else {
              // Fallback: Basic scoring
              const basicScore = calculateBasicScore(answer.answer);
            ...
            console.error('AI analysis failed for answer:', error);
            // Fallback: Basic scoring
            const basicScore = calculateBasicScore(answer.answer);
            totalScore += basicScore;
            analysisResults.push({ score: basicScore, feedback: 'Basic analysis applied' });
          ...
          // Empty answer gets 0
          analysisResults.push({ score: 0, feedback: 'No answer provided' });
        }
      }
      
      const averageScore = answers.length > 0 ? Math.round(totalScore / answers.length) : 0;
      
      // Store analysis results for detailed feedback
      window.lastAnalysisResults = analysisResults;
      
      return averageScore;
    } catch (error) {
      console.error('Error in calculateScore:', error);
      // Complete fallback to basic scoring
      return calculateBasicScore(answers.map(a => a.answer).join(' '));
    }
  };

  // Fallback scoring method
  const calculateBasicScore = (answerText) => {
    if (!answerText || answerText.trim().length === 0) {
      return 0;
    }
    
    let score = 40; // Base score for providing an answer
    ...
    if (wordCount >= 20 && wordCount <= 150) {
      ...
      score += 15; // Okay length
    } else if (wordCount > 150) {
      score += 10; // Too long but still effort
    }
    
    // Basic technical keywords (simple check)
    const technicalKeywords = [
      'javascript', 'react', 'node', 'api', 'database', 'function', 'component', 
      'state', 'props', 'css', 'html', 'express', 'mongodb', 'frontend', 'backend',
      'authentication', 'security', 'performance', 'responsive', 'async', 'await'
    ];
    ...
    score += Math.min(foundKeywords * 3, 20); // Max 20 bonus points for keywords
    
    // Completeness bonus (sentences and punctuation)
    const sentences = answerText.split(/[.!?]+/).filter(s => s.trim().length > 0).length;
    if (sentences >= 2) {
      score += 10;
    }
